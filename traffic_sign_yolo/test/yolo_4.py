...
class YOLOGUI(QMainWindow):
    ...
    def _build_ui(self):
        ...
        self.video_panel.setStyleSheet("""
            ...
            border-radius: 5px;
        """)
        self.video_panel.setMinimumSize(600, 450)  # Sửa từ setFixedSize thành setMinimumSize
        ...
        self.scroll_area.setStyleSheet("""
            QScrollArea {
                ...
                border: 1px solid #dfe6e9; 
                border-radius: 5px;
                min-height: 400px;  # Tăng từ 350px lên 400px
            }
            QScrollBar:vertical {
                border: none;
                background: #f7f9fb;
    ...
    def _on_sign_click(self, label):
        ...
        if os.path.exists(sample_path):
            ...
            img = img.resize((new_width, new_height), Image.Resampling.BICUBIC).convert("RGB")
            img_array = np.array(img)
            qimg = QImage(img_array.data, img_array.shape[1], img_array.shape[0], img_array.strides[0], QImage.Format_RGB888)
            self.sample_panel.setPixmap(QPixmap.fromImage(qimg).scaled(target_size, target_size, Qt.KeepAspectRatio, Qt.FastTransformation))
        else:
            ...
            self.sample_panel.setPixmap(self.default_image.scaled(150, 150, Qt.KeepAspectRatio, Qt.FastTransformation))
    ...
    def update_gui(self):
        try:
            ...
            if not self.result_queue.empty():
                ...
                canvas_width = 350  # Tăng từ 310 lên 350
                max_columns = 2   # Giảm từ 3 xuống 2
                padding = 8
                ...
                scaled_pixmap = QPixmap.fromImage(qimg).scaled(600, 450, Qt.KeepAspectRatio, Qt.FastTransformation)
